...
def create_registration():
    ...
    reg = Registration(
        reg_id=reg_id, name=name, roll=roll, year=year,
        dept=dept, phone=phone or None, email=email or None, college=college
    )
    db.session.add(reg)
    db.session.commit()

    return jsonify({'success': True, 'registration': reg.to_dict()}), 201
...
def list_registrations():
    ...
    return jsonify({'success': True, 'registrations': [r.to_dict() for r in regs], 'count': len(regs)})
